$(document).ready(function(){ 
	$('.compact .bx_filter_section .bx_filter_block .toggle').on('click', function(e){
        var _this = $(this),
			...
			if ($(this).closest(".bx_filter_parameters_box").hasClass("active")) {
				...
				setTimeout(function(){
					var boxOffsetLeft =_this.offset().left,
						//boxOffsetLeft =_this.next(".bx_filter_block").offset().left,
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			else
			{
				...
				if( $(curProp).closest('.bx_filter_parameters_box.check_props').find(".more_button_place") ) {
					$(curProp).closest('.bx_filter_section').addClass('button_box');
				}
			}
		}
	}
...
JCSmartFilter.prototype.showDropDownPopup = function(element, popupId)
{
    ...
    $("#smartFilterDropDown"+popupId).css({'top':'auto', 'left':'auto'/*, 'width':$(element).css('width')*/});
