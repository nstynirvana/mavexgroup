...
$arParams["IS_AJAX"] = (CNext::checkAjaxRequest() && $arParams['SHOW_FORM'] == 'Y');
...
if(!$bError && \Bitrix\Main\Loader::includeModule('aspro.next'))
{
	if($arParams["IS_AJAX"] != 'Y' && !$isWebform)
	{
		$ob = new Aspro\Next\MarketingPopup();
		$rules = $ob->GetRules();
	...
	else
	{
		if($request['iblock_id'] && $request['id'])
		{
			...
			$arResult['ITEM'] = CNextCache::CIBLockElement_GetList(array(
				'CACHE' => array(
					"TAG" => CNextCache::GetIBlockCacheTag($request['iblock_id']),
					"MULTI" => "N"
				)),
				$arFilter, 
				false, 
