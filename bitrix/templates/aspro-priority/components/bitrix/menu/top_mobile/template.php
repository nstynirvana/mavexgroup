	...
	<div class="menu top">
		<ul class="top">
			<?foreach($arResult as $arItem):?>
				...
				if($expandedLink == $arItem['LINK'])
					continue;
				?>
				<?$bParent = $arItem['CHILD'] && $bShowChilds;?>
				<li<?=($arItem['SELECTED'] ? ' class="selected"' : '')?>>
					<a class="dark-color<?=($bParent ? ' parent' : '')?>" <?if(!isset($arItem["UF_SHOW_LINK"]) && $arItem["UF_SHOW_LINK"] != 1):?>href="<?=$arItem["LINK"]?>"<?endif;?> title="<?=$arItem["TEXT"]?>">
						...
						<ul class="dropdown<?=(!$arItem['CHILD'] ? ' wtc' : '')?>">
							...
							<?foreach($arItem['CHILD'] as $arSubItem):?>
								...
								<li<?=($arSubItem['SELECTED'] ? ' class="selected"' : '')?>>
									<a class="dark-color<?=($bParent ? ' parent' : '')?>" <?if(!isset($arSubItem["UF_SHOW_LINK"]) && $arSubItem["UF_SHOW_LINK"] != 1):?>href="<?=$arSubItem["LINK"]?>"<?endif;?> title="<?=$arSubItem["TEXT"]?>">
