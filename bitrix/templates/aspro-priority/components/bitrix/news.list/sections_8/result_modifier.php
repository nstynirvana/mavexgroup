...
if($arResult['SECTIONS']){
	...
	if($arParams['SHOW_CHILD_SECTIONS'] === 'Y'){
		...
		$arIDParentSections = array();
		foreach($arResult['SECTIONS'] as $sectID => $section) {
			$arIDParentSections[] = $section["ID"];
			$arChildsSectionsFilter['SECTION_ID'] = $arIDParentSections;
		}
	...
	if(CPriority::GetFrontParametrValue('USE_REGIONALITY') === 'Y' && $arRegion && CPriority::GetFrontParametrValue('REGIONALITY_FILTER_ITEM') === 'Y'){
	...
	foreach($arResult['SECTIONS'] as $SID => &$arSection){
		...
		if(!$arSection['ELEMENTS_COUNT'] && CPriority::GetFrontParametrValue('USE_REGIONALITY') === 'Y' && $arRegion && CPriority::GetFrontParametrValue('SHOW_SECTIONS_REGION') == 'Y'){
		...
		} else { // elements if no childs sections
			if( $arParams['SHOW_CHILD_SECTIONS'] === 'Y' && $arParams['SHOW_ELEMENTS_IN_LAST_SECTION'] !== 'N' && $arSection['ELEMENTS_COUNT']) {
