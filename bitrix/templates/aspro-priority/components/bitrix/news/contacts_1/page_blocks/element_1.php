...
if($bUseMap && $arItem['PROPERTY_MAP_VALUE']){
	...
	if(strlen($arItem['PROPERTY_SCHEDULE_VALUE']['TEXT']) || $arItem['PROPERTY_PHONE_VALUE'] || $arItem['PROPERTY_METRO_VALUE'] || $arItem['PROPERTY_EMAIL_VALUE']){
		$html .= '<div class="properties">';
			...
			$html .= (strlen($arItem['PROPERTY_SCHEDULE_VALUE']['TEXT']) ? '<div class="property schedule"><div class="title-prop font_upper">'.$arProperties['SCHEDULE']['NAME'].'</div><div class="value font_sm">'.($arItem['PROPERTY_SCHEDULE_VALUE']['TYPE'] === 'HTML' ? str_replace(array("\r\n", PHP_EOL, "\n"), '', $arItem['PROPERTY_SCHEDULE_VALUE']['TEXT']) : $arItem['PROPERTY_SCHEDULE_VALUE']['TEXT']).'</div></div>' : '');
			...
			$html .= (strlen($arItem['PROPERTY_EMAIL_VALUE']) ? '<div class="property email"><div class="title-prop font_upper">'.$arProperties['EMAIL']['NAME'].'</div><div class="value font_sm"><a href="mailto:'.$arItem['PROPERTY_EMAIL_VALUE'].'">'.$arItem['PROPERTY_EMAIL_VALUE'].'</a></div></div>' : '');
		$html .= '</div>';
	}
...
<div class="contacts front contacts_page clearfix type_2 maxwidth-theme" itemscope itemtype="http://schema.org/Organization">
	<div class="row margin0">
		<div class="contacts__wrapp flexbox">
			<div class="item padding0 <?=($bUseMap ? 'col-md-6' : 'col-md-12')?>">
				<div class="left_block padding0 <?=($bUseMap ? '' : 'margin0')?>">
						...
						<div class="properties">
							...
							<?if(in_array('SCHEDULE', $arParams['LIST_PROPERTY_CODE']) && $arItem['PROPERTY_SCHEDULE_VALUE']):?>
								<div class="property schedule">
									...
									<div class="value"><?=$arItem['PROPERTY_SCHEDULE_VALUE']['TEXT'];?></div>
								</div>
							<?endif;?>
							...
							<?endif;?>
							<?global $arSite;?>
							<span class="hidden" itemprop="name"><?=$arSite["NAME"];?></span>
						</div>
					...
					<div class="right_block">
						<?if (CPriority::GetFrontParametrValue('CONTACTS_TYPE_MAP') == 'GOOGLE') {?>
							...
							array(
								...
								"MAP_DATA" => serialize(array("google_lat"=>$arCoords[0],"google_lon"=>$arCoords[1],"google_scale"=>16,"PLACEMARKS"=>$arPlacemarks)),
								"MAP_HEIGHT" => "650",
								"MAP_ID" => "",
								"MAP_WIDTH" => "100%",
								"OPTIONS" => array(
