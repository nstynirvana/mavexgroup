...
?>
<?if($arResult['ITEMS']):?>
	<div class="item-views catalog sections type_4_within type_12_within icons services-items list-4">
		...
		<div class="items row margin0 list_block">
			<?foreach($arResult['ITEMS'] as $arItem):?>
				...
				if($bShowSectionImage = in_array('PREVIEW_PICTURE', $arParams['FIELD_CODE'])){
					...
					$bBackground = ($arItem['PROPERTIES']['ICON_BACKGROUND']['VALUE_XML_ID'] == 'Y' ? true : false);
				}
				?>
				<div class="col-md-12 col-sm-12">
					<div class="item border shadow<?=($imageSectionSrc ? '' : ' wti')?><?=($bBackground ? ' wbg' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
