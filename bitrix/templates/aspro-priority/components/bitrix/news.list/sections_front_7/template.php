...
<?if($arResult['SECTIONS']):?>
	<div class="item-views services-items type_1 template_7 front icons<?=(isset($arParams['SCROLL_CLASS']) && $arParams['SCROLL_CLASS'] ? ' '.$arParams['SCROLL_CLASS'] : '')?> <?=$bCompact ? 'compact-view' : ''?>">
				...
				<div class="right_wrap col-md-9 col-sm-8">
					<div class="items row">
						...
						<?foreach($arResult['SECTIONS'] as $arItem):?>
							...
							?>
							<div class="item<?=($bShowSectionImage && strlen($imageSectionSrc) ? '' : ' wti')?><?=($bBackground ? ' wbg' : '');?><?=($bFirst ? ' active' : '')?><?=($bArrows ? ' warrows' : '')?> col-md-12 col-sm-12 <?=($bShowSectionImage && strlen($imageSectionSrc) ? '' : ' wti')?> <?=$arParams['IMAGE_CATALOG_POSITION'];?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
								<div class="wrap clearfix">
									<div class="wrap border">
										<?// icon or preview picture?>
										<?if($bShowSectionImage && strlen($imageSectionSrc)):?>
											<div class="image"><a href="<?=($isElements ? $arItem['DETAIL_PAGE_URL'] : $arItem['SECTION_PAGE_URL']);?>"><img src="<?=$imageSectionSrc?>" alt="<?=( $arItem['PICTURE']['ALT'] ? $arItem['PICTURE']['ALT'] : $arItem['NAME']);?>" title="<?=( $arItem['PICTURE']['TITLE'] ? $arItem['PICTURE']['TITLE'] : $arItem['NAME']);?>" class="img-responsive" /></a></div>
										<?endif;?>
										...
										<div class="body-info">
											...
											<?if(in_array('NAME', $arParams['FIELD_CODE'])):?>
												<div class="title">
													<a href="<?=($isElements ? $arItem['DETAIL_PAGE_URL'] : $arItem['SECTION_PAGE_URL']);?>" class="dark-color">
														<?=$arItem['NAME']?>
													</a>
												</div>
											<?endif;?>

											<?if(in_array('PREVIEW_TEXT', $arParams['FIELD_CODE']) && strlen($isElements ? $arItem['PREVIEW_TEXT'] : $arItem['DESCRIPTION'])):?>
												<div class="previewtext"><?=($isElements ? $arItem['PREVIEW_TEXT'] : $arItem['DESCRIPTION']);?></div>
											<?endif;?>
