	...
	<div class="wrap">
		<div class="wrapper_rel">
			<?if($arTheme['HEADER_MOBILE_MENU_OPEN']['VALUE'] == 1):?>
				...
				<?$showLight = $arTheme['HEADER_MOBILE_MENU_COLOR']['VALUE'] == "DARK";?>
				<div class="logo-wrapper">
					<div class="logo<?=$logoClass?>">
						<?=CPriority::ShowLogo($showLight);?>
					</div>
				</div>
			<?endif;?>
			...
			<?
			global $arRegion;
			if($arRegion || $arTheme['CABINET']['VALUE'] === 'Y'):?>
				<div class="top-info">
					<?
					// show regions
					CPriority::ShowMobileRegions();
					// show cabinet item
					CPriority::ShowMobileMenuCabinet();
					?>
				</div>
			<?endif;?>
			<?if(CPriority::nlo('menu-mobile', 'class="loader_circle" style="height:70px;"')):?>
			<!-- noindex -->
			<?$APPLICATION->IncludeComponent(
				"bitrix:menu",
				"top_mobile_expanded",
				Array(
					"COMPONENT_TEMPLATE" => "top_mobile_expanded",
			...
			<!-- /noindex -->
			<?endif;?>
			<?CPriority::nlo('menu-mobile');?>
			<div class="actions <?=$arTheme['HEADER_MOBILE_MENU_EXPAND']['VALUE'] ? 'order4' : ''?>">
				<?
				// show basket item
