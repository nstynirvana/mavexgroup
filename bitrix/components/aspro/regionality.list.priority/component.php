...
<?if(\Bitrix\Main\Loader::includeModule('aspro.priority'))
{
	...
	if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.priority', 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
	{
	...
	elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.priority', 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
	{
		$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
		$this->IncludeComponentTemplate();
	}
