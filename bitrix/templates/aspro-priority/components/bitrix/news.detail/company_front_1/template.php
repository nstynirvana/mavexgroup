				...
				<?if($bImage):?>
					<div class="item col-md-6">
						<div class="image" style="background:url(<?=$imageSrc;?>) top center / cover no-repeat;"<?=($bShowVideo ? ' data-video_source="'.$videoSource.'"' : '')?><?=(strlen($videoPlayer) ? ' data-video_player="'.$videoPlayer.'"' : '')?><?=(strlen($videoPlayerSrc) ? ' data-video_src="'.$videoPlayerSrc.'"' : '')?><?=($bVideoAutoStart ? ' data-video_autoplay="1"' : '')?>>
							<?if($bVideo):?>
								<div class="play">
									<div class="fancybox" rel="nofollow">
										<?if($arResult['DISPLAY_PROPERTIES']['VIDEO_SOURCE']['VALUE_XML_ID'] == 'PROPERTY'):?>
										...
										<?elseif($arResult['DISPLAY_PROPERTIES']['VIDEO_SOURCE']['VALUE_XML_ID'] == 'LINK'):?>
											<?//if($bVideoPlayerYoutube):?>
												<iframe width="100%" height="100%"  id="company_video_iframe" class="video'<?=($bVideoCover ? ' cover' : '');?>" data-src="<?=$videoPlayerSrc;?>" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen allow="accelerometer; encrypted-media; gyroscope; picture-in-picture"></iframe>
										<?/*elseif($bVideoPlayerVimeo):?>
											<iframe id="player_' + videoID + '" class="video' + (bVideoCover ? ' cover' : '') + '" src="'+ videoPlayerSrc +'" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
										<?elseif(bVideoPlayerRutube):?>
											<?
