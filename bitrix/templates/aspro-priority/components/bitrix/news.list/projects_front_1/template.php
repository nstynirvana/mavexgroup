...
<div class="item-views news-items projects type_1 front greyline">
	<div class="maxwidth-theme">
		<? if ($arParams['PAGER_SHOW_ALL']): ?>
			<a class="show_all pull-right" href="<?= str_replace('#SITE' . '_DIR#', SITE_DIR, $arResult['LIST_PAGE_URL']) ?>"><span>
					<?= (strlen($arParams['SHOW_ALL_TITLE']) ? $arParams['SHOW_ALL_TITLE'] : Loc::getMessage('S_TO_SHOW_ALL_PROJECTS')) ?>
				</span></a>
		<? endif; ?>
		<h2>
			<?= ($arParams["TITLE"] ? $arParams["TITLE"] : Loc::getMessage("TITLE_PROJECTS")); ?>
		</h2>
		...
		<div class="items row flexbox">
			...
			<? foreach ($arResult['ITEMS'] as $i => $arItem): ?>
				...
				// big block
				$bBigBlock = ($index == 1 || $index == 6 ? true : false);
				...
				?>
				<div
					class="item clearfix<?= ($bBigBlock ? ' big_block' : '') ?><?= (!$bImage ? ' wti' : '') ?><?= ($textColor ? ' ' . $textColor : '') ?> col-md-<?= ($bBigBlock ? 6 : 3) ?> col-sm-4 col-xs-6">
