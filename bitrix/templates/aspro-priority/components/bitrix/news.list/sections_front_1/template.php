...
<?if($arResult['SECTIONS']):?>
	<div class="item-views services-items type_1 front<?=(isset($arParams['SCROLL_CLASS']) && $arParams['SCROLL_CLASS'] ? ' '.$arParams['SCROLL_CLASS'] : '')?> <?=$bCompact ? 'compact-view' : ''?>">
					...
					<div class="items row">
						...
						<?foreach($arResult['SECTIONS'] as $arItem):?>
							...
							if($bShowSectionImage = in_array('PREVIEW_PICTURE', $arParams['FIELD_CODE'])){
								...
								$bImage = ($isElements ? strlen($arItem['~PREVIEW_PICTURE']) : strlen($arItem['~PICTURE']));
								$arSectionImage = ($bImage ? CFile::ResizeImageGet($isElements ? $arItem['~PREVIEW_PICTURE'] : $arItem['~PICTURE'], array('width' => 615, 'height' => 10000), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : array());
