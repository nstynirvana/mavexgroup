...
if(!function_exists("ShowFilterItemExt")){
	function ShowFilterItemExt($key, $arItem, $col, $arParams){?>
		<?
		global $APPLICATION;		
		if($key!="TOP_BLOCK"){
		...
		$countElementsShow = $arParams['COUNT_IN_FILTER'];
		?>
		<div class="col-md-<?=$col;?>">
			<div class="bx_filter_parameters_box <?=$class;?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code=<?=strtolower($arItem["CODE"]);?> property_id="<?=$arItem["ID"]?>">
				...
				<?if($arItem["CODE"]!="IN_STOCK"){?>
					<div class="bx_filter_parameters_box_title<?=(strlen($arItem['FILTER_HINT']) ? ' whint' : '')?>">
						<span>
							<?=($arItem["NAME"] == GetMessage('FILTER_PRICE') ? GetMessage("PRICE") : $arItem["NAME"]);?>
							<?if(strlen($arItem['FILTER_HINT'])):?>
								<span class="hints">
									...
									<div class="tooltip"><span><?=$arItem['FILTER_HINT'];?></span></div>
								</span>
							<?endif;?>
						</span>
							...
							switch ($arItem["DISPLAY_TYPE"]){
								...
								case "H"://CHECKBOXES_WITH_PICTURES_AND_LABELS
									...
									<?foreach ($arItem["VALUES"] as $val => $ar):?>
										...
										<?if($index == $countElementsShow - 1 && $arItem["VALUES"] > $countElementsShow):?>
											<?$hiddenBlock = true;?>
									...
									<?if($hiddenBlock):?>
										...
										<div class="toggle btn btn-xs btn-default" data-open_text="<?=GetMessage('OPEN_TEXT');?>" data-close_text="<?=GetMessage('CLOSE_TEXT');?>"><span><?=GetMessage('SHOW_ALL');?></span></div>
									<?endif;?>
									<?
									break;
								case "P"://DROPDOWN
								...
								case "R"://DROPDOWN_WITH_PICTURES_AND_LABELS
									?>
									<div class="bx_filter_select_container wpicture">
										<div class="bx_filter_select_block s_<?=CUtil::JSEscape($key)?>" data-id="<?=CUtil::JSEscape($key)?>" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
											<div class="bx_filter_select_text" data-role="currentOption">
												<?
												$checkedItemExist = false;
								...
								case "K"://RADIO_BUTTONS
									...
									<?$i = 1;$hiddenBlock = false;?>
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										<?if($i > $countElementsShow && !$hiddenBlock):?>
											<?$hiddenBlock = true;?>
											<div class="hidden_block">
												<div class="wrap">
										<?endif;?>
								...
								case "U"://CALENDAR
									?>
									<div class="bx_filter_parameters_box_container_block date_input">
