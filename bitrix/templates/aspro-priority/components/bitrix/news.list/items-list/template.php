...
<div class="item-views <?=$arParams['VIEW_TYPE']?> <?=$arParams['VIEW_TYPE']?>-type-block <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>">
	...
	<?endif;?>
		...
		<?endif;?>
				...
				<div class="licenses_block <?=($arParams['SHOW_TABS'] == 'Y' ? 'tab-content' : 'group-content')?>">
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?endif;?>
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									// preview picture
									$bImage = strlen($arItem['FIELDS']['PREVIEW_PICTURE']['SRC']) || strlen($arItem['FIELDS']['DETAIL_PICTURE']['SRC']);
									...
									$arFile = CPriority::get_file_info($arItem['FIELDS']['DETAIL_PICTURE']['ID']);
									$fileSize = CPriority::filesize_format($arFile['FILE_SIZE']);
									?>
									<?ob_start();?>
										<?// element name?>
										...
										<?if(strlen($arItem['FIELDS']['PREVIEW_TEXT']) || strlen($arItem['FIELDS']['DETAIL_TEXT'])):?>
											<div class="button" style="display:none;"><span class="opener font_upper"><?=GetMessage('OPEN_TEXT');?></span></div>
										<?endif;?>
									...
									<?ob_start();?>
										<?if($bImage):?>
											<div class="image <?=($bImage ? ' w-picture' : ' wo-picture wpi')?>">
												...
												<?endif;?>
													<?
													$arItem['FIELDS']['PREVIEW_PICTURE'] = ($arItem['FIELDS']['PREVIEW_PICTURE'] ? $arItem['FIELDS']['PREVIEW_PICTURE'] : ($arItem['FIELDS']['DETAIL_PICTURE'] ? $arItem['FIELDS']['DETAIL_PICTURE'] : ''));
													?>
													<?$arImg = CFile::ResizeImageGet($arItem['FIELDS']['PREVIEW_PICTURE']['ID'], array('width' => 10000, 'height' => 144), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true);?>
													<img src="<?=$arImg['src'];?>" alt="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-responsive" />
												<?if($bDetailLink):?>
													</a>
												<?elseif($arItem['FIELDS']['DETAIL_PICTURE']):?>
													<span class="zoom">
									...
									<?if($arParams['VIEW_TYPE'] == 'list'):?>
										<div class="col-md-12">
											<div class="item border shadow<?=($bImage ? '' : ' wti')?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
												<?if($bImage):?>
													<?=$imagePart?>
												<?endif;?>
												<div class="body-info">
													...
													<?if($arItem['FIELDS']['DETAIL_PICTURE']):?>
														<a href="<?=$imageDetailSrc?>" class="fancybox"><span class="zoom">
															<?=CPriority::showIconSvg(SITE_TEMPLATE_PATH.'/images/include_svg/zoom.svg');?>
														</span></a>
													<?endif;?>
												</div>
