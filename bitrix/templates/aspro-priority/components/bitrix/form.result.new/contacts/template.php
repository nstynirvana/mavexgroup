				...
				<div class="form contacts<?=($arResult['FORM_NOTE'] ? ' success' : '')?><?=($arResult['isFormErrors'] == 'Y' ? ' error' : '')?>  item-views blocks">
					...
					$(document).ready(function(){
						...
						$('.form .add_file').on('click', function(){
							var index = $(this).closest('.input').find('input[type=file]').length;
							var inputName = $(this).closest('.input').find('input[type="file"]').prop('name').split('_')[0];
							$('<input type="file" id="POPUP_FILE" name="' + inputName + '_n'+index+'"   class="inputfile" value="" />').insertBefore($(this));
							$('input[type=file]').uniform({fileButtonHtml: BX.message('JS_FILE_BUTTON_NAME'), fileDefaultHtml: BX.message('JS_FILE_DEFAULT')});
						})
					});
					</script>
