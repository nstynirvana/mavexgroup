		...
		<ul class="nav nav-list side-menu">
			<?foreach($arResult as $arItem):?>
				...
				<li class="item<?=($arItem["SELECTED"] ? " active opened" : "")?> <?=($arItem["CHILD"] ? " child" : "")?>">
					<a  <?if(!isset($arItem["UF_SHOW_LINK"]) && $arItem["UF_SHOW_LINK"] != 1):?>href="<?=$arItem["LINK"]?>"<?endif;?>><?=(isset($arItem["PARAMS"]["BLOCK"]) && $arItem["PARAMS"]["BLOCK"] ? $arItem["PARAMS"]["BLOCK"] : "");?><?=$arItem["TEXT"]?><?=(isset($arItem['CHILD']) && $arItem['CHILD'] ? '<span class="arrow'.($arItem['SELECTED'] ? ' opened' : '').'"></span>' : '')?></a>
					<?ShowSubItems($arItem);?>
				</li>
			<?endforeach;?>
		</ul>
