					...
					<div class="items row flexbox">
						<?foreach($arResult['SECTIONS'] as $arItem):?>
							...
							if($bShowSectionImage = in_array('PREVIEW_PICTURE', $arParams['FIELD_CODE'])){
								$bImage = strlen($isElements ? $arItem['~PREVIEW_PICTURE'] : $arItem['~PICTURE']);
								$arSectionImage = ($bImage ? CFile::ResizeImageGet($isElements ? $arItem['~PREVIEW_PICTURE'] : $arItem['~PICTURE'], array('width' => 735, 'height' => 10000), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true) : array());
							...
							<div class="item col-md-4 col-sm-4 col-xs-6 <?=($bShowSectionImage && $imageSectionSrc ? '' : ' wti')?> <?=$arParams['IMAGE_CATALOG_POSITION'];?>" data-id="<?=$arItem['ID'];?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
								<div class="wrap shadow">
									...
									<?if($bShowSectionImage && $imageSectionSrc):?>
										<div class="image">
											<div class="wrap"><img src="<?=$imageSectionSrc?>" alt="<?=($arItem['PREVIEW_PICTURE']['ALT'] ?? $arItem['NAME']);?>" title="<?=( $arItem['PREVIEW_PICTURE']['TITLE'] ?? $arItem['NAME']);?>" class="img-responsive" /></div>
										</div>
									<?endif;?>
										...
										<div class="wrap">
											...
											<?if( in_array('PREVIEW_TEXT', $arParams['FIELD_CODE']) && strlen($isElements ? $arItem['PREVIEW_TEXT'] : $arItem['DESCRIPTION']) ):?>
												<div class="bottom-block">
													<div class="previewtext font_xs"><?=CPriority::truncateLengthText($isElements ? $arItem['PREVIEW_TEXT'] : $arItem['DESCRIPTION'], $arParams['PREVIEW_TRUNCATE_LEN'])?></div>
												</div>
											<?endif?>
										</div>
									</div>
