				...
				<div class="item-views front blocks instagram_scroll">
					<div class="maxwidth-theme">
						...
						<div class="instagram type_3 clearfix">
							<?$index = 0;?>
							<?$arInstagramPosts['data'][0]['LINK'] = $arInstagramPosts['data'][0]['thumbnail_url']  ? $arInstagramPosts['data'][0]['thumbnail_url'] : $arInstagramPosts['data'][0]['media_url'];?>
							<div class="items clearfix">
								<div class="left_item">
									<div class="item">
										<div class="image" style="background:url(<?=$arInstagramPosts['data'][0]['LINK'];?>) center center/cover no-repeat;"><a href="<?=$arInstagramPosts['data'][0]['permalink']?>" target="_blank" rel="nofollow"></a></div>
										<div class="desc">
											<div class="wrap">
												<div class="date font_upper"><?=CPriority::showIconSvg(SITE_TEMPLATE_PATH.'/images/include_svg/instagram_mainpage.svg');?><span><?=FormatDate('d F', strtotime($arInstagramPosts['data'][0]['timestamp']), 'SHORT');?></span></div>
												<?if($arInstagramPosts['data'][0]['caption']):?>
													<div class="text font_xs"><?=$arInstagramPosts['data'][0]['caption'];?></div>
												<?endif;?>
												<a href="<?=$arInstagramPosts['data'][0]['permalink']?>" target="_blank" rel="nofollow"></a>												
											</div>
										</div>
									</div>
								</div>
								<div class="right_items_1">
									<?foreach($arInstagramPosts['data'] as $key => $arItem):?>
										<?
										if($key == 0){
											continue;
										}
