...
$iVisibleItemsMenu = CPriority::GetFrontParametrValue('MAX_VISIBLE_ITEMS_MENU') ?? 10;
...
$sCountElementsMenuWide = "count_".CPriority::GetFrontParametrValue('COUNT_ITEMS_IN_LINE_MENU_WIDE');
$sCountElementsMenuFull = "count_".CPriority::GetFrontParametrValue('COUNT_ITEMS_IN_LINE_MENU_FULL');
...
<?if($arResult):?>
	<div class="menu-only<?=($bImagePositionTop ? ' image_top' : '')?><?=($bShowIcons ? ' with_icons' : '')?> catalog_icons_<?=CPriority::GetFrontParametrValue('SHOW_CATALOG_SECTIONS_IMAGE');?> icons_position_<?=$sIconPosition?> view_type_<?=$sViewTypeMenu?> <?=(in_array($headerType, $arFullHeaderValues) ? 'count_menu_full_'.$sCountElementsMenuFull : 'count_menu_wide_'.$sCountElementsMenuWide)?>">
													...
													<?if($bShowImage):?>
														...
														<?if($arPicture):?>
															<div class="image<?=($bBackground ? ' wbg' : '');?><?=($bShowIcons && !$arSubItem['PARAMS']['ICON_BACKGROUND'] ? ' wtbg' : '');?>">
																<a href="<?=$arSubItem["LINK"]?>"><img src="<?=$arPicture['src']?>" alt="<?=$arSubItem['TEXT']?>" title="<?=$arSubItem['TEXT']?>" /></a>
															</div>
														<?endif?>
													<?endif;?>
													<div class="menu_body<?=($bShowImage && $arPicture ? ' with_image' : '')?>">
														<a class="dark-color" <?if(!isset($arSubItem["UF_SHOW_LINK"]) && $arSubItem["UF_SHOW_LINK"] != 1) : ?>href="<?=$arSubItem["LINK"]?>"<?endif;?> title="<?=$arSubItem["TEXT"]?>"><?=$arSubItem["TEXT"]?><?=($arSubItem["CHILD"] && $bShowChilds ? '<span class="arrow"><i></i></span>' : '')?></a>
														<?if($arSubItem["CHILD"] && $bShowChilds):?>
															<ul class="dropdown-menu">
																...
																<?foreach($arSubItem["CHILD"] as $arSubSubItem):?>																
																	...
																	<li class="<?=($arSubSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=(++$i > $iVisibleItemsMenu ? 'collapsed' : '');?> <?=($arSubSubItem["SELECTED"] ? "active" : "")?>" <?=($i > $iVisibleItemsMenu ? 'style="display:none;"' : '');?>>
