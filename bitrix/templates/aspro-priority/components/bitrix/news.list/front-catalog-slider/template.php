...
<?if($arResult['SECTIONS'] || $arResult['ITEMS']):?>
	...
	<div class="catalog item-views table front catalog_block_main">
		<div class="row margin0">
			<div class="flexslider unstyled row dark-nav" data-plugin-options='{"useCSS": false, "animation": "slide", "directionNav": true, "controlNav": true, "animationLoop": true, "slideshow": false, "itemMargin": 0, "counts": [<?=$countmd?>, <?=$countsm?>, <?=$countxs?>, <?=$countxs1?>]}'>
					...
					<?foreach($arResult["ITEMS"] as $i => $arItem):?>
						...
						<li class="item-wrap col-md-<?=$colmd?> col-sm-<?=$colsm?> col-xs-<?=$colxs?>">
							<div class="item<?=($bShowImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>"<?=($bOrderViewBasket ? ' data-item="'.$dataItem.'"' : '')?> itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product">
								...
								<?if($arItem['DISPLAY_PROPERTIES']['BRAND']):?>
									<?$brandItem = current($arItem['DISPLAY_PROPERTIES']['BRAND']["LINK_ELEMENT_VALUE"]);?>
									<meta itemprop="brand" content="<?=$brandItem["NAME"]?>" />
								<?endif;?>
											...
											<div class="footer-button">
												<?if($bOrderButton):?>
													...
													<?if($bOrderButton && !$bOrderViewBasket):?>
														<span class="btn btn-default animate-load" <?=(strlen(($arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE']) && strlen($arItem['DISPLAY_PROPERTIES']['PRICEOLD']['VALUE'])) ? 'style="margin-top:16px;"' : '')?> data-event="jqm" data-param-id="<?=CPriority::getFormID("aspro_priority_order_product");?>" data-product="<?=$arItem["NAME"]?>" data-name="order_product"><?=(strlen($arParams['S_ORDER_PRODUCT']) ? $arParams['S_ORDER_PRODUCT'] : GetMessage('TO_ORDER'))?></span>
													<?endif;?>
													<?// element buy block?>
