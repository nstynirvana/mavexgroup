...
if($arItems)
{
		...
		if($arParams['USE_FILTER'] != 'N')
	{
		...
		<div class="wrapper-filter-date news-list-4 clearfix">
			<?if(\Bitrix\Main\Loader::includeModule('subscribe')):?>
					<div class="subscribe font_upper pull-right">
						<span class="animate-load" data-event="jqm" data-param-id="subscribe" data-param-type="subscribe" data-name="subscribe">
							...
							<?=( $arParams['S_SUBSCRIBE'] && !empty($arParams['S_SUBSCRIBE']) ? $arParams['S_SUBSCRIBE'] : GetMessage('SUBSCRIBE_BUTTON') );?>
						</span>
					</div>
			<?endif;?>
			<div class="head-block top clearfix arrow_scroll">
				<?if($arParams["SHOW_FILTER_DATE"] === "Y"):?>
					<div class="tabs-wrapper">
						<ul class="nav nav-tabs date hidden-xs">
							<li class="item-link font_upper_md<?=(!$yearGet ? ' active' : '')?>">
								<div class="title">
									<?if(!$yearGet):?>
							...
							<?foreach($arYears as $value):?>
								...
								?>
								<li class="item-link font_upper_md<?=($bSelected ? ' active' : '')?>">
			...
			if($bHasYear)
			{
				$GLOBALS[$arParams["FILTER_NAME"]][">DATE_ACTIVE_FROM"] = ConvertDateTime("31.12.".($year-1), FORMAT_DATE,'');
				$GLOBALS[$arParams["FILTER_NAME"]]["<=DATE_ACTIVE_FROM"] = ConvertDateTime("31.12.".$year, FORMAT_DATE,'');
			}?>
		<?}
	}
}?>
...
<div class="row news_list_4">
	<div class="col-md-<?=($arSections ? 9 : 12);?> news_list_4">
