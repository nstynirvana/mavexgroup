	...
	<div class="maxwidth-theme">
		...
		<div class="top_block clearfix">
			<? if ($arParams['PAGER_SHOW_ALL']): ?>
				<a class="show_all pull-right"
					href="<?= str_replace('#SITE' . '_DIR#', SITE_DIR, $arResult['LIST_PAGE_URL']) ?>"><span>
						<?= (strlen($arParams['SHOW_ALL_TITLE']) ? $arParams['SHOW_ALL_TITLE'] : Loc::getMessage('S_TO_SHOW_ALL_NEWS')) ?>
					</span></a>
			<? endif; ?>
			<div class="pull-left top_title_block">
				<h2>
					<?= ($arParams["TITLE"] ? $arParams["TITLE"] : Loc::getMessage("TITLE_NEWS")); ?>
				</h2>
			</div>
		...
		<div class="items row">
			...
			<? foreach ($arResult['ITEMS'] as $i => $arItem): ?>
				...
				?>
				<div
					class="item<?= ($bBigBlock ? ' big_block' : '') ?><?= (!$bImage ? ' wti' : '') ?> col-md-<?= ($bBigBlock ? 6 : 3) ?> col-sm-4 col-xs-6">
