...
<?if($arResult['ITEMS']):?>
	...
	?>
	<div class="item-views front tarifs wicons type_2 type_8 tarifs_scroll">
								...
								<div class="wrap">
										...
										<div class="prices">
											<ul class="all_price">
												<?foreach($arItem['PRICES'] as $keyPrice => $arPrice):?>
													<li class="price<?=($defaultPrice == $arPrice['VALUE'] ? ' active' : '');?>" data-price="<?=$arPrice['VALUE'];?>" data-filter_price="<?=$arItem['FILTER_PRICES'][$keyPrice];?>" data-name="<?=$arPrice['NAME'];?>">
									...
									<?if($arItem['PROPERTIES']['FORM_ORDER']['VALUE_XML_ID'] == 'YES' && !$bOrderViewBasket):?>
										<div class="order<?=($bOrderViewBasket ? ' basketTrue' : '')?>">
											<?if($arItem['PROPERTIES']['FORM_ORDER']['VALUE_XML_ID'] == 'YES' && !$bOrderViewBasket):?>
												<span class="btn btn-default btn-xs animate-load btn-transparent" data-event="jqm" data-param-id="<?=CPriority::getFormID("aspro_priority_order_product");?>" data-name="order_product" data-autoload-product="<?=$arItem['NAME']?><?=($defaultPrice ? ': '.$defaultPrice : '');?>">
													<span><?=(strlen($arParams['S_ORDER_PRODUCT']) ? $arParams['S_ORDER_PRODUCT'] : GetMessage('S_ORDER_PRODUCT'))?></span>
												</span>
											<?endif;?>
										</div>
									<?endif;?>
