...
$sectionsCount = $arParams['NEWS_COUNT'];
	...
	if(CPriority::GetFrontParametrValue('SHOW_SECTIONS_REGION') == 'Y' && CPriority::GetFrontParametrValue('REGIONALITY_FILTER_ITEM') == 'Y' && $arRegion){
		...
		if($arElements){
			foreach($arElements as $arElement){
				if( $sectionsCount && count($arSectionsID) >= $sectionsCount) {
					break;
				}
				if($arElement['IBLOCK_SECTION_ID']){
					$arSectionsID[ $arElement['IBLOCK_SECTION_ID'] ] = $arElement['IBLOCK_SECTION_ID'];
				}
			}
