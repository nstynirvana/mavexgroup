...
<div class="mail_soc_wrapper" style="text-align:center;padding-top:20px;font-size:0px;">
	...
	<?if(!empty($arResult['SOCIAL_ZEN'])):?>
		<a href="<?=$arResult['SOCIAL_ZEN']?>" target="_blank" class="mail_soc" title="<?=GetMessage('ZEN')?>">
			<img src="/bitrix/components/aspro/social.info.next/images/zen.png" alt="<?=GetMessage("ZEN")?>" title="<?=GetMessage("ZEN")?>" />
		</a>
	<?endif;?>
	<?if(!empty($arResult['SOCIAL_TIKTOK'])):?>
		<a href="<?=$arResult['SOCIAL_TIKTOK']?>" target="_blank" class="mail_soc" title="<?=GetMessage('TIKTOK')?>">
			<img src="/bitrix/components/aspro/social.info.next/images/tiktok.png" alt="<?=GetMessage("TIKTOK")?>" title="<?=GetMessage("TIKTOK")?>" />
		</a>
	<?endif;?>
	<?if(!empty($arResult['SOCIAL_PINTEREST'])):?>
		<a href="<?=$arResult['SOCIAL_PINTEREST']?>" target="_blank" class="mail_soc" title="<?=GetMessage('PINTEREST')?>">
			<img src="/bitrix/components/aspro/social.info.next/images/pinterest.png" alt="<?=GetMessage("PINTEREST")?>" title="<?=GetMessage("PINTEREST")?>" />
		</a>
	<?endif;?>
	<?if(!empty($arResult['SOCIAL_SNAPCHAT'])):?>
		<a href="<?=$arResult['SOCIAL_SNAPCHAT']?>" target="_blank" class="mail_soc" title="<?=GetMessage('SNAPCHAT')?>">
			<img src="/bitrix/components/aspro/social.info.next/images/snapchat.png" alt="<?=GetMessage("SNAPCHAT")?>" title="<?=GetMessage("SNAPCHAT")?>" />
		</a>
	<?endif;?>
	<?if(!empty($arResult['SOCIAL_LINKEDIN'])):?>
		<a href="<?=$arResult['SOCIAL_LINKEDIN']?>" target="_blank" class="mail_soc" title="<?=GetMessage('LINKEDIN')?>">
			<img src="/bitrix/components/aspro/social.info.next/images/linkedin.png" alt="<?=GetMessage("LINKEDIN")?>" title="<?=GetMessage("LINKEDIN")?>" />
		</a>
	<?endif;?>
</div>