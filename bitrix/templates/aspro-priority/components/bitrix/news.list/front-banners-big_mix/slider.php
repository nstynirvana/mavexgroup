...
$touch = (count($arResult['SECTIONS']['BIG']['ITEMS']) > 1 ? "" : '"touch": false');
...
?>
<div class="banners-big wmix_banner front<?=($bHideOnNarrow ? ' hidden_narrow' : '')?> view_<?=$bannerMobile?>">
	<div class="maxwidth-banner">
		<div class="flexslider unstyled <?=($animationDirection == 'vertical' ? 'vertical' : '')?>" data-plugin-options='{"directionNav": true, <?=$touch?>, "customDirection": ".nav-carousel a", "controlNav": true, "nav" : "normal", <?=($bAnimation && empty($touch) ? '"slideshow": true,' : '"slideshow": false,')?> <?=($animationType ? '"animation": "'.$animationType.'",' : '')?> <?=($animationDirection ? '"direction": "'.$animationDirection.'",' : '')?> <?=($slideshowSpeed >= 0 ? '"slideshowSpeed": '.$slideshowSpeed.',' : '')?> <?=($animationSpeed >= 0 ? '"animationSpeed": '.$animationSpeed.',' : '')?> "animationLoop": true}'>
								...
								<?if($bannerMobile == 2 || $bannerMobile == 3):?>
										<?if($bannerMobile == 2):?>
											...
											<div class="wrap" >
												<?if(strlen($tablet_text)):?>
													<div class="inner">
														<div class="tablet_img" <?=$bannerMobile == 2 ? 'style="background-image:url('.$imageBgSrc.') center center / cover no-repeat;"' : ''?>><?=$img?></div>
										...
										<?elseif($bannerMobile == 3):?>
											...
											<div class="tablet_img">
												<div style="background-image:url('<?=$tabletImgSrc?>');"></div>
													<?if($bShowVideo && !$bVideoAutoStart && ($bShowButton1 || $bShowButton2) && !$bOnlyImage):?>
														<span class="play btn-video <?=($bVideoAutoStart ? 'loading' : '');?> small <?=$buttonVideoClass;?>" title="<?=$buttonVideoText?>"></span>
													<?endif;?>
											</div>
										<?endif;?>
